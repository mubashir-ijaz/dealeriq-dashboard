...
// Dedicated CarMax tab — browse all CarMax purchases, filter by title status
// (Available / Unavailable / Unknown / …), switch between table/gallery view,
...
export default function CarMaxTab() {
  ...
  const handleExport = () => {
    const filename = statusFilter === 'all' ? 'CarMax_Cars' : `CarMax_${statusFilter.replace(/\s+/g, '_')}_Cars`;
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 14 }}>
        <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderTop: '2px solid var(--text3)', borderRadius: 12, padding: '16px 20px', boxShadow: 'var(--shadow-sm)' }}>
          ...
          <div style={{ fontSize: 11, color: 'var(--text2)', marginTop: 3, textTransform: 'uppercase', letterSpacing: '0.8px' }}>Total CarMax Vehicles</div>
        </div>
      ...
      {view === 'gallery' ? (
        <CarMaxGallery rows={paged} pageOffset={page * PAGE_SIZE} meta={meta} onOpen={setModalIndex} />
      ) : (
      ...
      {modalIndex !== null && filtered[modalIndex] && (() => {
        ...
        return (
          <CarDetailModal
            ...
            image={r.image}
            title={`${r.year || ''} ${r.make || ''} ${r.model || ''}`.trim() || 'CarMax vehicle'}
            subtitle={r.vin}
...
function CarMaxGallery({ rows, meta, pageOffset = 0, onOpen }) {
  if (rows.length === 0) {
